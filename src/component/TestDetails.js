...
const TestDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`13.201.191.84/api/testDrive/get/${id}`, {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    const fetchCarData = async (carId) => {
      ...
      const url = `13.201.191.84/api/cars/cars/${carIdString}`;
  ...
  const handleCancel = async () => {
    try {
      const response = await fetch(`13.201.191.84/api/testDrive/${id}/cancel`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  const confirmDelete =async () => {
    try {
      const response = await fetch(`13.201.191.84/api/testDrive/delete/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
